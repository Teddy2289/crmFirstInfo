            ...
            <div class="col-lg-6">
                <div class="card">
                    <div class="card-body">
                        ...
                        @can('add-permission')
                        <button type="button" class="btn btn-primary btn-rounded mb-3" wire:click="addpermission()">Add Permission</button>
                        @if ($form == 'addpermission')
                        <form class="my-2" wire:submit.prevent='storepermission'>
                            <div class="card">
                                <div class="card-body">
                                    <div class="mb-3">
                                        <label for="namepermission" class="form-label">Name Permission</label>
                                        <input type="text" class="form-control" id="namepermission" aria-describedby="emailHelp" name="namepermission" wire:model='namepermission'>
                        ...
                        <form class="mb-3" wire:submit.prevent='updatepermission({{ $idpermission }})'>
                            <div class="card mb-3">
                                <div class="card-body">
                                    <label for="editnamepermission" class="form-label">Name Permission</label>
                                    <input type="text" class="form-control" id="editnamepermission" aria-describedby="emailHelp" name="namepermission" wire:model='namepermission'>
                                    @error('namepermission')
                                    <div class="alert alert-danger" role="alert">
                                        {{ $message }}
                                    </div>
                            ...
                            <div class="col-md-12 mt-3 mb-3">
                                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#confirmationModalPermission">Submit</button>
                                <button type="button" class="btn btn-danger" wire:click='resetall'>Cancel</button>
                            </div>
                            ...
                            <!-- MODAL Edit -->
                            <div class="modal fade" id="confirmationModalPermission" tabindex="-1" aria-labelledby="confirmationModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="confirmationModalLabel">Confirmation</h5>
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                                            <button type="submit" class="btn btn-primary" onclick="document.querySelector('#editnamepermission').form.submit()">Confirm</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @can('edit-permission')
                                            <button type="button" class="btn btn-raised btn-rounded btn-raised-primary" wire:click="editPermission('{{ $item->id }}')"><i class="nav-icon i-Pen-2 font-weight-bold"></i></button>
                                            @endcan
                                            @can('delete-permission')
                                            <!-- <button type="button" class="btn btn-raised btn-rounded btn-raised-danger" onclick="if(confirm('Are you sure you want to delete this permission?')){Livewire.emit('confirmDeletePermission', '{{ $item->id }}')} return false;"><i class="nav-icon i-Close-Window font-weight-bold"></i></button> -->
                                            <button class="btn btn-raised btn-rounded btn-raised-danger" wire:click="deletePermissionConfirmation({{ $item->id }})" data-toggle="modal" data-target="#deleteConfirmationModalPermission"> <i class="nav-icon i-Close-Window font-weight-bold"></i></button>
                                            @endcan
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                                <div wire:ignore.self class="modal fade" id="deleteConfirmationModalPermission" tabindex="-1" role="dialog" aria-labelledby="deleteConfirmationModalLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="deleteConfirmationModalLabel">Confirm Delete</h5>
                                            ...
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                                                <button type="button" class="btn btn-danger" wire:click="deletePermissionConfirmed">Delete</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
